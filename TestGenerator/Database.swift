...
class Database {
    ...
    init() {
        //기본 문제생성, 법인 아닌 사단 SO
        let test0 = Test(category: "변호사시험", subject: "민사법", number: 6)
        
        let question0 = Question(test : test0, number : 32, questionKey : "TESTCVA00025", questionType: QuestionType.Select, questionOX : QuestionOX.X , content : "법인 아닌 사단에 관한 설명 중 옳지 않은 것은?", answer : 5)
...
//        
//        _ = question2.findAnswer()
//
//        self.questions.append(question2)
//        
        //기본 문제생성, 동시이행의 항변 FO
        
        let test3 = Test(category: "변호사시험", subject: "민사법", number: 3)
        
        let question3 = Question(test : test3, number : 12, questionKey : "TESTCVA00518", questionType: QuestionType.Find, questionOX : QuestionOX.O , content : "동시이행의 항변권에 관한 설명 중 옳은 것을 모두 고른 것은?", answer : 1)
        ...
        // ㄱ, 옳은 지문
        let selectionVeryFirst31 = Selection(question: question3, selectNumber: 0, content: "부동산매수인이 매매계약을 체결하면서 매매목적물에 관한 근저당권의 피담보채무를 인수하는 한편 그 채무액을 매매대금에서 공제하기로 하는 이행인수계약이 함께 이루어진 경우, 매수인의 인수채무 불이행으로 인한 손해배상채무와 매도인의 소유권이전등기의무는 동시이행의 관계에 있다.", selectString : "ㄱ")
        // ㄴ, 옳은 지문
        let selectionVeryFirst32 = Selection(question: question3, selectNumber: 0, content: "쌍무계약의 당사자 일방이 먼저 한 번 현실의 제공을 하고 상대방을 수령지체에 빠지게 하였다 하더라도 그 이행의 제공이 계속되지 않는 경우 상대방이 가지는 동시이행의 항변권이 소멸하는 것은 아니다.", selectString : "ㄴ")
        // ㄷ, 틀린 지문
        let selectionVeryFirst33 = Selection(question: question3, selectNumber: 0, content: "원고가 단순이행청구를 함에 대하여 피고가 동시이행의 항변권을 행사하지 않더라도 법원은 직권으로 상환이행판결을 할 수 있다.", selectString : "ㄷ")
        // ㄹ, 틀린 지문
        let selectionVeryFirst34 = Selection(question: question3, selectNumber: 0, content: "\"피고는 원고로부터 5,000만 원을 지급받음과 동시에 A토지를 인도하라.\"라는 판결을 받은 원고는 반대의무 이행 또는 이행의 제공을 하였다는 것을 증명하여야만 집행을 개시할 수 있다.", selectString : "ㄹ")
...
//        
//        //기본 문제생성, 매도인의 담보책임 SX
//        let question5 = Question(questionKey : "TESTCVA00516", isPublished : true, testDate : "1310", testCategory: "변모", testSubject: "민사법", questionType: QuestionType.Select, questionOX : QuestionOX.Correct , content : "甲은 어느 날 乙로부터 책 한 권과 함께 \"이 책이 마음에 드시면 아래 계좌에 대금을 납부해주십시오\"라는 서면을 받았다. 甲은 그 책을 읽다가 흥미를 느껴서 밑줄까지 그어가며 읽었고, 다 읽은 후에는 丙에게 \"참 좋은 책이니 너도 한 번 읽어봐라\"라는 편지와 함께 그 책을 보내주었다. 이 경우 甲과 乙 사이의 법률관계를 설명하는 방법으로 가장 적당한 것은?", answer : 4)
//        
//        // 1번, 옳은 지문
...
//        
//        //기본 문제생성, 동시이행의 항변 FO
//        let questionFX = Question(questionKey : "TESTCVA00522", isPublished : true, testDate : "1406", testCategory: "변모", testSubject: "민사법", questionType: QuestionType.Find, questionOX : QuestionOX.X , content : "동시이행의 항변권에 대한 판례의 태도에 부합하지 않는 설명을 모두 고른 것은?", answer : 2)
...
//        
//        
//        let selectionVeryFirstFX1 = Selection(question: questionFX, selectNumber: 0, content: "동시이행관계에 있는 채무에 관하여 지체책임이 발생하지 아니하는 효과는 항변권의 행사를 필요로 하지 아니한다.", selectString : "ㄱ")
//        let selectionVeryFirstFX2 = Selection(question: questionFX, selectNumber: 0, content: "다른 사정이 없으면 매매대금채무와 재산권이전의무는 동시이행의 관계에 있으므로 어느 일방의 채무에 관한 이행의 제공이 있기 전에는 소멸시효가 진행되지 않는다.", selectString : "ㄴ")
//        let selectionVeryFirstFX3 = Selection(question: questionFX, selectNumber: 0, content: "임대차 종료 시 임차인의 임차보증금반환채권은 임차건물에 관해 생긴 것이 아니므로 그 임차건물의 반환과 동시이행의 관계에 있지 아니하다.", selectString : "ㄷ")
//        let selectionVeryFirstFX4 = Selection(question: questionFX, selectNumber: 0, content: "상대방의 적법한 이행제공을 수령하지 아니하여 수령지체에 빠진 쌍무계약의 일방 당사자는 동시이행의 항변권을 더 이상 원용할 수 없다.", selectString : "ㄹ")
//        let selectionVeryFirstFX5 = Selection(question: questionFX, selectNumber: 0, content: "매도인의 소유권이전등기의무가 이행불능임을 이유로 매매계약을 해제하기 위해서 매수인은 그것과 동시이행관계에 있는 잔대금지급의무의 이행제공을 할 필요가 없다.", selectString : "ㅁ")
//        
...
//        
//        _ = questionFX.findAnswer()
//        self.questions.append(questionFX)
    }
