...
struct StatementView: View {
    ...
    var body: some View {
        HStack {
            VStack() {
                Text(showNumber != nil ? "\(showNumber!.roundInt)" : "  ")
                Spacer()
            }
            VStack() {
                let wrappedText = statement.content.map { String($0) }.joined(separator: " ")
...
struct StatementView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        return StatementView(statement: statement, showNumber: 3)
    }
}
